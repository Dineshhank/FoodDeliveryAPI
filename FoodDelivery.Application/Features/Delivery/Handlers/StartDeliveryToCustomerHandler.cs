...
{
    public class StartDeliveryToCustomerHandler : IRequestHandler<StartDeliveryToCustomerCommand, bool>
    {
        private readonly IOrderRepository _orderRepository;
        private readonly IDeliveryRepository _deliveryRepository;

        public StartDeliveryToCustomerHandler(
        ...
        public async Task<bool> Handle(
            StartDeliveryToCustomerCommand request,
            CancellationToken cancellationToken)
        {
            ...
            if (order.Status != DeliveryOrderStatuses.PickedUp)
                return false;

            var now = DeliveryDbTime.UnspecifiedNow();
            order.Status = DeliveryOrderStatuses.OnTheWayToCustomer;
            order.Updatedat = now;
            ...
            await _orderRepository.AddOrderStatusHistoryAsync(new Orderstatushistory
            {
                Id = Guid.NewGuid(),
                Orderid = order.Id,
                Status = DeliveryOrderStatuses.OnTheWayToCustomer,
